...
- **[GitHub MCP Server](https://github.com/github/github-mcp-server)** — GitHub's official MCP server: issues, pull requests, code search, Actions, and more, exposed as agent tools. The Day 1 appetizer ([cross-link](./resources-day1.md#github-mcp-server)) — register it locally or use the hosted endpoint to let agents work directly against your repos.
- **[Microsoft Learn MCP Server](https://github.com/MicrosoftDocs/mcp)** — First-party MCP server that gives agents grounded, real-time access to Microsoft Learn documentation (Azure, .NET, PowerShell, M365). Endpoint: `https://learn.microsoft.com/api/mcp`. Cuts hallucinated APIs by letting the agent cite current docs.
